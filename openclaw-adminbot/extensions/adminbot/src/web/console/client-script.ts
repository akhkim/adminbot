 ...
 *
 * Runs in the operator's browser, so it is plain browser JS inside a template literal rather than a
 * module: the console is served as one self-contained document with no build step of its own. The
 * interpolations inject server-side constants the client would otherwise have to re-fetch.
 */
import {
...
} from "../../workflows/members/collaborator-subgroups.js";
