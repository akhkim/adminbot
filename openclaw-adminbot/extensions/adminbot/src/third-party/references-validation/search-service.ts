...
import {
  ...
  formatDblpISO690,
  generateDblpBibTeX,
} from "./dblp-service.js";
import {
  searchOpenAlex,
  formatOpenAlexAPA,
  generateOpenAlexBibTeX,
  formatOpenAlexMLA,
...
import {
  ...
  generateSemanticScholarBibTeX,
  formatSemanticScholarMLA,
  formatSemanticScholarISO690,
} from "./semantic-scholar-service.js";
...
const extractLikelyTitle = (rawRef: string): string | null => {
  ...
  if (segments.length > 1) {
    ...
    const venueKeywords =
      /\b(journal|proceedings|conference|transactions|advances|letters|review|annals|bulletin|workshop|symposium|arxiv)\b/i;
...
export const computeAuthorSim = (expectedAuthorsStr: string, resultAuthors: string[]): number => {
  if (!expectedAuthorsStr || resultAuthors.length === 0) {
    return 100;
  }
  ...
  for (const a of resultAuthors) {
    ...
    if (family.length > 1) {
      // > 1 to handle 2-letter last names like Ho, Wu, Li
      validAuthors++;
...
const formatAPA = (item: any): string => {
  const authors = (item.author || [])
    .map(
      (a: { family: string; given?: string }) => `${a.family}, ${a.given ? a.given[0] + "." : ""}`,
    )
    .join(", ");
...
const formatMLA = (item: any): string => {
  ...
  if (authors.length === 1) {
    ...
    authorsStr =
      `${authors[0].family}, ${authors[0].given || ""}, and ${authors[1].given || ""} ${authors[1].family}`.trim();
  } else if (authors.length > 2) {
...
const generateBibTeX = (item: any): string => {
  ...
  const authors = (item.author || [])
    .map((a: { family: string; given?: string }) => `${a.family}${a.given ? ", " + a.given : ""}`)
    .join(" and ");
  const year = item.published?.["date-parts"]?.[0]?.[0] || "n.d.";
  const title = item.title?.[0] || "Untitled";
  const journal = item["container-title"]?.[0] || "";
...
export const checkReference = async (
  rawQuery: string,
  expectedInput?: ExpectedMetadata,
  ...
  try {
    ...
    const expected = expectedInput
      ? {
          ...expectedInput,
          title: expectedInput.title ? cleanQuery(expectedInput.title) : expectedInput.title,
          authors: expectedInput.authors
            ? cleanQuery(expectedInput.authors)
            : expectedInput.authors,
          journal: expectedInput.journal
            ? cleanQuery(expectedInput.journal)
            : expectedInput.journal,
        }
      : expectedInput;
    ...
    if (items.length > 0) {
      ...
      for (const item of items) {
        ...
        if (expected?.title) {
          ...
          if (nTitle && (nQuery.includes(nTitle) || nTitle.includes(nQuery))) {
            ...
            const wordOverlap =
              titleWords.length > 0 ? matchingTitleWords.length / titleWords.length : 0;
            ...
            const reverseOverlap =
              queryWords.length > 0 ? matchingQueryWords.length / queryWords.length : 0;
        ...
        if (expected?.year) {
          if (iYears.includes(expected.year)) {
            ...
            iPrimaryYear &&
            Math.abs(Number.parseInt(expected.year, 10) - Number.parseInt(iPrimaryYear, 10)) <= 1
          ) {
      ...
      const resultAuthors = (item.author || [])
        .map((a: { family: string; given?: string }) => a.family)
        .join(", ");
      ...
      const realAuthorFamilies: string[] = (item.author || [])
        .map((a: { family: string; given?: string }) => normalize(a.family || ""))
      ...
      const realAuthorGivens: string[] = (item.author || [])
        .map((a: { family: string; given?: string }) => (a.given || "").trim())
      ...
      if (expected?.title) {
        ...
        if (expected.authors) {
          ...
          if (authorsFound.length > 0) {
            if (hasEtAlBatch) {
              ...
              authorSim = Math.min(
                100,
                Math.round((authorsFound.length / realAuthorFamilies.length) * 100),
              );
            }
          } else {
          ...
          for (const expAuth of expectedList) {
            ...
            const matchesFamily = realAuthorFamilies.some(
              (real) => real.includes(nExp) || nExp.includes(real),
            );
            const matchesGiven = realAuthorGivens.some((given) =>
              firstLetterMatches(expAuth, given),
            );

            if (!matchesFamily && !matchesGiven) {
              // Don't flag "et al" or "others" as a fake author
              if (
                !expAuth.toLowerCase().includes("et al") &&
                !expAuth.toLowerCase().includes("others") &&
                !expAuth.toLowerCase().includes("al.")
              ) {
        ...
        if (expected.journal) {
          ...
          const publisherNames =
            /\b(springer|elsevier|wiley|routledge|sage|cambridge|oxford|harvard|princeton|edward\s+elgar|mcgraw.hill|pearson|academic\s+press|lexington\s+books?|university\s+press|palgrave|macmillan|taylor\s+&?\s*francis|ieee|acm)\b/gi;
          ...
          if (journalSim < 50) {
            ...
            if (expectedIsPreprint !== resultIsPreprint && titleSim > 70) {
              ...
              issues.push(
                `Version difference: "${expected.journal}" vs "${resultJournal}" (preprint/published)`,
              );
        ...
        if (expected.year && resultYear) {
          ...
          if (expected.year === resultYear || resultAllYears.includes(expected.year)) {
            ...
            const yearDiff = Math.abs(
              Number.parseInt(expected.year, 10) - Number.parseInt(resultYear, 10),
            );
            const eitherIsPreprint =
              isPreprint(expected.journal || "") || isPreprint(resultJournal);

            if (yearDiff <= 2 && eitherIsPreprint) {
              yearSim = 85;
              issues.push(
                `Year differs by ${yearDiff} year(s): expected ${expected.year}, found ${resultYear} (likely preprint/published version)`,
              );
            } else if (yearDiff <= 2 && titleSim > 80) {
              // Even without preprint label, ±2 year with high title match is likely online-first vs print
              yearSim = 85;
              issues.push(
                `Note: expected ${expected.year}, CrossRef shows ${resultYear} (likely online-first vs print date)`,
              );
            } else if (yearDiff === 1 && titleSim > 70) {
              yearSim = 75;
              issues.push(
                `Year differs by 1: expected ${expected.year}, found ${resultYear} (possible version difference)`,
              );
            } else {
              yearSim = 0;
        ...
        const allContainerTitles = (item["container-title"] || [])
        ...
        for (const token of queryTokens) {
          ...
          if (nToken.length < 4) {
            continue;
          } // Skip short tokens (initials, "et", "al")
          if (!/^[A-Z]/.test(token)) {
            continue;
          } // Only check capitalized tokens
          if (/^\d+$/.test(token)) {
            continue;
          } // Skip pure numbers
          if (stopWords.has(nToken)) {
            continue;
          } // Skip known non-author words

          // Check against result title
          if (nResultTitle.includes(nToken)) {
            continue;
          }
          // Check against result journal
          if (resultJournal && normalize(resultJournal).includes(nToken)) {
            continue;
          }
          // Check against result year
          if (resultYear && resultYear.includes(token)) {
            continue;
          }
          // Check against publisher
          if (resultPublisher && resultPublisher.includes(nToken)) {
            continue;
          }
          // Check against all container titles (book series, etc.)
          if (allContainerTitles && allContainerTitles.includes(nToken)) {
            continue;
          }

          // Check if it's a real author (family name) — also check substrings for compound names
          // e.g. "Silva" should match "da Silva Meireles"
          const isRealAuthorFamily = realAuthorFamilies.some(
            (real) =>
              real.includes(nToken) ||
              nToken.includes(real) ||
              // Also check individual words of multi-word family names
              real.split(/\s+/).some((part) => part === nToken),
          );
          const isRealAuthorGiven = realAuthorGivens.some((given) =>
            firstLetterMatches(token, given),
          );
        ...
        if (resultJournal && resultJournal.length > 3) {
          const journalWords = normalize(resultJournal)
            .split(/\s+/)
            .filter((w) => w.length >= 4);
        ...
        if (resultYear && resultYear.length === 4) {
          ...
          if (
            yearsInQuery.includes(resultYear) ||
            yearsInQuery.some((y) => resultAllYears.includes(y))
          ) {
            // Year matches one of the CrossRef date fields - good!
          } else if (yearsInQuery.length > 0) {
            const yearDiff = Math.abs(
              Number.parseInt(yearsInQuery[0], 10) - Number.parseInt(resultYear, 10),
            );
            const resultIsPreprint = isPreprint(resultJournal);

            if (yearDiff <= 2 && resultIsPreprint) {
              issues.push(
                `Note: year ${yearsInQuery[0]} vs ${resultYear} (preprint/published version difference)`,
              );
              overallSim -= 5;
            } else if (yearDiff <= 2 && titleSim > 80) {
              // Online-first vs print, common 1-2 year gap
              issues.push(
                `Note: year ${yearsInQuery[0]} vs ${resultYear} (likely online-first vs print date)`,
              );
              overallSim -= 5;
            } else if (yearDiff === 1 && titleSim > 70) {
              issues.push(
                `Year differs by 1: you wrote ${yearsInQuery[0]}, actual is ${resultYear} (possible version difference)`,
              );
              overallSim -= 10;
            } else {
      ...
      const isRetracted =
        nResultTitle.includes("retract") ||
        nResultTitle.includes("withdraw") ||
        (item["update-to"] &&
          Array.isArray(item["update-to"]) &&
          item["update-to"].some((u: { type?: string }) => u.type === "retraction"));
...
const scoreCandidateResult = (
  ...
  if (expectedYear && resultYear) {
    if (resultYear === expectedYear) {
      score += 40;
    } else if (Math.abs(Number.parseInt(expectedYear, 10) - Number.parseInt(resultYear, 10)) <= 1) {
      score += 15;
    } else if (Math.abs(Number.parseInt(expectedYear, 10) - Number.parseInt(resultYear, 10)) <= 2) {
      score += 5;
    }
  }
 ...
 */
const checkWithFallbackInternal = async (
  query: string,
  expected?: ExpectedMetadata,
  originalQuery?: string,
): Promise<CheckResult> => {
    ...
    if (arxivFromDoi) {
      ...
      if (arxivDirectResult) {
        return {
          ...
          journal: arxivDirectResult.category
            ? `arXiv [${arxivDirectResult.category}]`
            : "arXiv preprint",
  ...
  if (arxivId && !queryDOI) {
    // Skip if we already tried via DOI
    const arxivDirectResult = await resolveArxivById(arxivId);
    if (arxivDirectResult) {
      const titleSim = expected?.title
        ? computeTitleSim(expected.title, arxivDirectResult.title)
        : 90;
  ...
  if (expectedYear && Number.parseInt(expectedYear, 10) > currentYear + 1) {
  ...
  if (expected?.authors) {
    const firstAuthor = expected.authors
      .split(/,| and /)[0]
      .trim()
      .split(/\s+/)
      .pop();
    if (firstAuthor && firstAuthor.length > 1) {
      // > 1 handles 2-letter names like "Ho", "Li", "Wu"
  ...
  const crossRefPromise = checkReference(
    crossRefQuery,
    expected,
    originalQuery || (expected ? undefined : query),
  );
    ...
    if (primaryBest.result.authorMatchScore >= 70 && primaryBest.titleSim >= 85) {
      if (primaryBest.source === "CrossRef") {
        ...
        if (otherBest && otherBest.score > primaryBest.score) {
          ...
          winner.issues = [
            `Better match found in ${otherBest.source} (CrossRef confidence was ${primaryBest.result.matchConfidence}%)`,
          ];
          return winner;
        }
      ...
      if (crossRefResult.exists) {
        winner.issues = [
          `Better match found in ${primaryBest.source} (CrossRef confidence was ${crossRefResult.matchConfidence}%)`,
        ];
      }
      if (futureYearIssues.length > 0) {
  ...
  if (arxivResult) {
    ...
    if (arxivTitleSim >= MIN_TITLE_SIMILARITY) {
      const arxivScore = scoreCandidateResult(
        arxivTitleSim,
        arxivAuthorSim,
        arxivResult.year?.toString(),
        arxivResult.category,
        expectedYear,
        expectedJournal,
      );
  ...
  if (dblpResult) {
    ...
    if (dblpTitleSim >= MIN_TITLE_SIMILARITY) {
      const dblpScore = scoreCandidateResult(
        dblpTitleSim,
        dblpAuthorSim,
        dblpResult.year?.toString(),
        dblpResult.venue,
        expectedYear,
        expectedJournal,
      );
  ...
  if (candidates.length > 0) {
    ...
    if (best.source === "CrossRef") {
      ...
      if (otherBest && otherBest.score > best.score) {
        const winner = otherBest.result;
        winner.issues = [
          `Better match found in ${otherBest.source} (CrossRef confidence was ${best.result.matchConfidence}%)`,
        ];
        return winner;
      }
      if (futureYearIssues.length > 0) {
    ...
    if (crossRefResult.exists) {
      winner.issues = [
        `Better match found in ${best.source} (CrossRef confidence was ${crossRefResult.matchConfidence}%)`,
      ];
    }
    if (futureYearIssues.length > 0) {
      winner.issues = [...winner.issues, ...futureYearIssues];
    ...
    if (winner.titleMatchScore < MIN_TITLE_SIMILARITY) {
      return {
        ...
        journalMatchScore: 0,
        issues: [
          "Title not found in any source (CrossRef, Semantic Scholar, OpenAlex, DBLP, arXiv)",
        ],
      };
    }
    return winner;
...
export const checkWithFallback = checkWithFallbackInternal;
