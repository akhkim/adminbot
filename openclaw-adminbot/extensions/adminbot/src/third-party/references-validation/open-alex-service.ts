  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      for (const work of data.results) {
        ...
        if (expectedYear && work.publication_year) {
          if (work.publication_year.toString() === expectedYear) {
            score += 20;
          } else if (Math.abs(work.publication_year - Number.parseInt(expectedYear, 10)) === 1) {
            score += 5;
          }
        }
      ...
      return {
        ...
        isRetracted:
          bestWork.is_retracted ||
          bestWork.title.toLowerCase().includes("retract") ||
          bestWork.title.toLowerCase().includes("withdraw"),
      };
    }
