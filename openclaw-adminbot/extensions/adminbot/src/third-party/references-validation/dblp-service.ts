...
const normalizeAuthors = (authorData: any): string[] => {
  ...
  return authors
    .map((a: unknown) => {
      ...
      if (typeof a === "string") {
        name = a;
      } else if (typeof a === "object" && a !== null && "text" in a) {
        name = (a as { text: string }).text;
      } else {
        name = String(a);
      }
...
export const searchDblp = async (
  ...
  try {
    ...
    for (const hit of hits) {
      ...
      const hitYear = info.year ? Number.parseInt(info.year, 10) : null;
      if (expectedYear && hitYear) {
        if (hitYear.toString() === expectedYear) {
          score += 20;
        } else if (Math.abs(hitYear - Number.parseInt(expectedYear, 10)) === 1) {
          score += 5;
        }
      }
