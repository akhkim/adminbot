...
export class AdminBotService {
  ...
  private async sendSlackChannelNamingNotice(
    ...
    return { ok: true, status: 200, payload: this.store.getProposal(proposal.payload.id) ?? proposal.payload };
  }
...
function evaluateSlackChannelName(params: {
  ...
  return {
    valid,
    expectedPrefix,
    suggestedName: SLACK_CHANNEL_NAME_RE.test(suggestedName) ? suggestedName : `${expectedPrefix}-channel`,
  };
}
