...
describe("AdminBotAuthService claim/login flow", () => {
  ...
  describe("DCS form submission on approval", () => {
    it("submits the approved member's own name and email, split into first/last", async () => {
      const calls: Array<{ firstName: string; lastName: string; email: string }> = [];
      const { auth } = setup({
        submitDcsForm: async (params) => {
          calls.push(params);
        },
      });
      const signup = auth.signup({
    ...
    // the single word fills both questions.
    it("uses a one-word name for both first and last name", async () => {
      const calls: Array<{ firstName: string; lastName: string }> = [];
      const { store, auth } = setup({
        submitDcsForm: async (params) => {
          calls.push(params);
        },
      });
      store.saveLabMember(member("cher", "cher@example.com", { name: "Cher" }));
      auth.claim({ member_id: "cher", email: "cher@example.com", password: "correcthorse" });
      auth.approveRegistration(pendingIdForMember(auth, "cher"), "admin");

      await Promise.resolve();
      await Promise.resolve();
      expect(calls).toEqual([{ firstName: "Cher", lastName: "Cher", email: "cher@example.com" }]);
    });

    // A name with a middle name/initial: everything but the last token is the first name, so
    // "Mary Jane Watson" splits as "Mary Jane" / "Watson", not "Mary" / "Jane Watson".
    it("keeps a middle name with the first name, not the last", async () => {
      const calls: Array<{ firstName: string; lastName: string }> = [];
      const { store, auth } = setup({
        submitDcsForm: async (params) => {
          calls.push(params);
      ...
      store.saveLabMember(member("mjw", "mjw@example.com", { name: "Mary Jane Watson" }));
      auth.claim({ member_id: "mjw", email: "mjw@example.com", password: "correcthorse" });
      auth.approveRegistration(pendingIdForMember(auth, "mjw"), "admin");

      await Promise.resolve();
      await Promise.resolve();
      expect(calls).toEqual([
        { firstName: "Mary Jane", lastName: "Watson", email: "mjw@example.com" },
      ]);
    });

    it("does not fail or block approval when the form submission itself fails", async () => {
      const { auth } = setup({
        submitDcsForm: async () => {
      ...
      expect(signup.ok).toBe(true);
      expect(approved.ok).toBe(true);
      // The rejection is fire-and-forget; letting it settle here just proves it does not throw
      // out of approveRegistration or leave an unhandled rejection.
      await Promise.resolve();
      await Promise.resolve();
    });
