...
import type { AdminBotEvidencePointer } from "../../contracts/actions.js";
...
/**
 * The two named calendar destinations, keyed by the env var that identifies each one. They name a
 * specific Google Workspace, so they are deployment configuration rather than tracked constants.
 *
 * Unset, that named destination is simply unavailable: `resolveCalendarSource` logs which var to
 * set and proposes no `calendar_id`, which is the same shape as a caller that named no calendar at
 * all. Failing the whole action would break every calendar proposal that carries an explicit
 * `calendarUrl` instead.
 */
export const ADMINBOT_CALENDAR_ENV = {
  personal: "ADMINBOT_CALENDAR_ID",
  jinesis: "ADMINBOT_BOT_EMAIL",
} as const;

export function resolveNamedCalendarId(
  name: "personal" | "jinesis",
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const varName = ADMINBOT_CALENDAR_ENV[name];
  const value = env[varName]?.trim();
  if (!value) {
    console.warn(
      `[adminbot] calendar "${name}" is not configured: set ${varName}. Proposing no calendar_id.`,
    );
    return undefined;
  }
  return value;
...
export async function resolveCalendarSource(
  ...
  const calendarId =
    ...
    (params.calendarName ? resolveNamedCalendarId(params.calendarName, env) : undefined);
