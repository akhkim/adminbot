...
//
// Why this exists: the copy in `onboarding-emails.ts` carries no hard wrapping, and neither does
// the compose path -- but the operator still received these mails with ~70-character breaks
// mid-paragraph. The breaks are introduced at delivery, by sending a `text/plain` part: a long
// logical line is quoted-printable soft-wrapped on the way out and then re-wrapped again by the
// reading client, and neither of those is something the sender controls. A `text/html`
// alternative is, so every send path renders one from the same string it already has.
//
...
//
// It lives beside the gog executor because it is a property of *delivery*, and because every
// caller that sends already imports from this file.
