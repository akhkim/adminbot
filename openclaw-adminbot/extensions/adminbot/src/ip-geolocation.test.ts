...
describe("createIpinfoLiteGeolocator", () => {
  it("looks up a public IP against the IPinfo Lite endpoint with the token", async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ country: "Switzerland", country_code: "CH", continent: "Europe", continent_code: "EU" }),
    );
    const geolocate = createIpinfoLiteGeolocator("test-token", fetchImpl as unknown as typeof fetch);
  ...
  it("treats the 172.16.0.0/12 boundary correctly", async () => {
    ...
    const skipped = vi.fn();
    const geolocateSkipped = createIpinfoLiteGeolocator("test-token", skipped as unknown as typeof fetch);
    ...
    const geolocateCalled = createIpinfoLiteGeolocator("test-token", called as unknown as typeof fetch);
