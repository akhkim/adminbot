...
placeholders: { zhijing_whatsapp }
policy_note: |
  The supplied text carries Zhijing's real WhatsApp number. Repo policy is explicit --
  "Never commit real phone numbers" (AGENTS.md, Security / Release) -- so it is a
  placeholder here and is resolved at send time from AdminBot settings
  (`head_professor_whatsapp`), never from this file. /settings is admin-gated on read
