...
import { parseInlineDirectives } from "../directives/directive-handling.parse.js";
import { handleConfigCommand, handleDebugCommand } from "./commands-config.js";
import type { HandleCommandsParams } from "./commands-types.js";
import type { ConfigSnapshotMock } from "./commands.test-harness.js";

// This lane runs with `isolate: false`, and the runner's per-file module reset
// only fires in `onAfterRunSuite` — a sibling that throws while collecting (see
// `commands-name.test.ts`) never gets there, so its real, unmocked module graph
// can still be cached when this file loads. Dropping the registry from a hoisted
// block runs before the static imports below bind, so the `vi.mock` factories in
// this file are what `commands-config.js` and its dependencies actually see.
vi.hoisted(() => {
  vi.resetModules();
});
...
vi.mock("../../../agents/tools/bash-tools.js", () => ({
...
vi.mock("../../../config/paths/config-paths.js", () => ({
...
vi.mock("../../../config/runtime/runtime-overrides.js", () => ({
...
vi.mock("../../../config/runtime/runtime-schema.js", async () => {
  const actual = await vi.importActual<typeof import("../../../config/schema/schema.js")>(
...
describe("command gating", () => {
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    ...
    expect(replaceConfigFileMock.mock.calls.length).toBe(previousWriteCount + 1);
    expect(replaceConfigFileMock.mock.calls.at(-1)?.[0]).toEqual({
      nextConfig: {},
