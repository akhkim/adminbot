...
async function finalizeAcpTurnOutput(params: {
  ...
  // to prove the final result was visible to the user. Without TTS the accumulated block text is
  // the only thing that can still need delivering.
  ...
  if (shouldDeliverTextFallback) {
    const delivered = await params.delivery.deliver("final", { text: accumulatedVisibleBlockText });
...
export async function tryDispatchAcpReply(params: {
  ...
  try {
    ...
    // Inbound media attachments went with the media-understanding subsystem, so an ACP turn is
    // now always plain prompt text.
    const attachments: never[] = [];
    const turnPromptText = promptText;
