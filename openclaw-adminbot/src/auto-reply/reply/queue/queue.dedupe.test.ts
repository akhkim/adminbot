...
describe("followup queue deduplication", () => {
  ...
  it("deduplicates same message_id across distinct enqueue module instances", async () => {
    const enqueueA = await importFreshModule<typeof import("./enqueue.js")>(
      import.meta.url,
      "./enqueue.js?scope=dedupe-a",
    );
    const enqueueB = await importFreshModule<typeof import("./enqueue.js")>(
      import.meta.url,
      "./enqueue.js?scope=dedupe-b",
    );
