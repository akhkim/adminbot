...
describe("defineToolPlugin", () => {
  ...
  it("registers plugin approvals for tools that require confirmation", async () => {
    const entry = defineToolPlugin({
      id: "approval-tools",
      name: "Approval Tools",
      description: "Approval tool demo.",
      tools: (tool) => [
        tool({
          name: "approval_echo",
          description: "Echo with approval.",
          parameters: Type.Object({ actionId: Type.String() }),
          approval: (params) => ({
            title: "Approve echo",
            description: `Approve ${params.actionId}`,
            severity: "warning",
    ...
    const captured = createCapturedPluginRegistration({ id: "approval-tools" });
    const on = vi.fn();
    ...
    await expect(
      handler(
        { toolName: "approval_echo", params: { actionId: "act_123" }, toolCallId: "call-1" },
        { agentId: "adminbot", sessionKey: "session-1" },
      ),
    ).resolves.toEqual({
      requireApproval: {
        title: "Approve echo",
        description: "Approve act_123",
        severity: "warning",
        allowedDecisions: ["allow-once", "deny"],
        timeoutBehavior: "deny",
      },
    ...
    await expect(
      handler({ toolName: "other_tool", params: {}, toolCallId: "call-2" }, {}),
