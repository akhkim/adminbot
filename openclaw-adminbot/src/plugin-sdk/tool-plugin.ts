...
type ToolPluginApprovalRequest = NonNullable<PluginHookBeforeToolCallResult["requireApproval"]>;

type ToolPluginApprovalContext = Pick<
  PluginHookToolContext,
  "agentId" | "sessionId" | "sessionKey" | "toolCallId" | "toolName"
>;

type ToolPluginApprovalResolver<TParamsSchema extends TSchema> = (
  params: Static<TParamsSchema>,
  context: ToolPluginApprovalContext,
) => ToolPluginApprovalRequest | undefined | Promise<ToolPluginApprovalRequest | undefined>;

type ToolPluginApproval<TParamsSchema extends TSchema> =
  | ToolPluginApprovalRequest
  | ToolPluginApprovalResolver<TParamsSchema>;
...
type ToolPluginToolDefinitionBase<TParamsSchema extends TSchema> = {
  ...
  optional?: boolean;
  /** Require OpenClaw plugin approval before the tool implementation is called. */
  approval?: ToolPluginApproval<TParamsSchema>;
};
...
type DefinedToolPluginTool = {
  ...
  parameters: TSchema;
  optional: boolean;
  approval?: ToolPluginApproval<TSchema>;
...
function createToolPluginToolFactory<TConfig>(): ToolPluginToolFactory<TConfig> {
  return ((definition: ToolPluginToolDefinition<TConfig, TSchema>) => ({
    ...
    approval: definition.approval as DefinedToolPluginTool["approval"],
...
async function resolveToolPluginApproval(params: {
  approval: ToolPluginApproval<TSchema>;
  eventParams: Record<string, unknown>;
  context: ToolPluginApprovalContext;
}): Promise<ToolPluginApprovalRequest | undefined> {
  if (typeof params.approval === "function") {
    return params.approval(params.eventParams, params.context);
  }
...
export function defineToolPlugin<TConfigSchema extends TSchema | undefined = undefined>(
  ...
  const entry = definePluginEntry({
    ...
    register(api) {
      ...
      const toolsRequiringApproval = tools.filter((tool) => tool.approval);
      if (toolsRequiringApproval.length > 0) {
        api.on("before_tool_call", async (event, ctx) => {
          const matchingTool = toolsRequiringApproval.find((tool) => tool.name === event.toolName);
          if (!matchingTool?.approval) {
            return undefined;
          }
          const requireApproval = await resolveToolPluginApproval({
            approval: matchingTool.approval,
