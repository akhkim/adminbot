...
describe("ChannelsSchema bundled runtime loading", () => {
  beforeEach(() => {
    ...
    vi.doMock("../../plugins/install/bundled-channel-config-metadata.js", () => ({
  ...
  it("skips bundled channel runtime discovery when only core channel keys are present", async () => {
    const runtime = await importFreshModule<typeof import("./channels-config.js")>(
      import.meta.url,
      "./channels-config.js?scope=channels-core-only",
    );
  ...
  it("does not discover bundled channel runtime metadata during raw schema parsing", async () => {
    const runtime = await importFreshModule<typeof import("./channels-config.js")>(
      import.meta.url,
      "./channels-config.js?scope=channels-plugin-owned",
    );
