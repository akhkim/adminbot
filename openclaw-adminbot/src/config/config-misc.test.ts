...
describe("gateway.controlUi.launchUrl", () => {
  it("accepts and normalizes hosted http(s) launch URLs", () => {
    const result = OpenClawSchema.safeParse({
      gateway: {
        controlUi: {
          launchUrl: "  https://control.example.com/admin?theme=dark  ",
        },
      },
    });
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.gateway?.controlUi?.launchUrl).toBe(
        "https://control.example.com/admin?theme=dark",
      );
    }
  });

  it("rejects unsupported or credential-bearing launch URLs", () => {
    for (const value of [
