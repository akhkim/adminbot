...
export const FIELD_HELP: Record<string, string> = {
  ...
  "gateway.controlUi.launchUrl":
    "Optional hosted Control UI URL opened by setup and `openclaw dashboard` instead of the gateway-served bundled UI. OpenClaw adds the Gateway WebSocket URL as `gatewayUrl` and token auth as a URL fragment at launch time; configure `gateway.controlUi.allowedOrigins` for that hosted origin separately.",
  "gateway.auth":
