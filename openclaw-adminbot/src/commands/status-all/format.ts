...
export function resolveStatusDashboardUrl(params: {
  ...
  const configuredLaunchUrl = resolveControlUiLaunchUrl(params.cfg.gateway?.controlUi?.launchUrl);
  if (!(params.cfg.gateway?.controlUi?.enabled ?? true) && !configuredLaunchUrl) {
    return null;
  }
  ...
  return buildControlUiLaunchUrl({
