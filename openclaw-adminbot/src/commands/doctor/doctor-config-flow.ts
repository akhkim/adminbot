...
import { CONFIG_PATH } from "../../config/paths/paths.js";
...
import { emitDoctorNotes, sanitizeDoctorNote } from "./emit-notes.js";
...
} from "./shared/default-account-warnings.js";
import { normalizeCompatibilityConfigValues } from "./shared/legacy-config-core-migrate.js";
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  const pluginLegacyIssues = await (async () => {
    ...
    const { findDoctorLegacyConfigIssues } = await import("./shared/legacy-config-issues.js");
  ...
  const { applyPluginAutoEnable } = await import("../../config/plugin/plugin-auto-enable.js");
  ...
  const { collectPluginToolAllowlistWarnings } =
    await import("./shared/plugin-tool-allowlist-warnings.js");
  ...
  let collectMutableAllowlistWarnings:
    | typeof import("./shared/channel-doctor.js").collectChannelDoctorMutableAllowlistWarnings
    | undefined;
  if (hasConfiguredChannels) {
    const channelDoctor = await import("./shared/channel-doctor.js");
  ...
  const { repairHooksTokenReuseGatewayAuth } = await import("./shared/hooks-token-reuse-repair.js");
  ...
  if (shouldRepair) {
    const { runDoctorRepairSequence } = await import("./repair-sequencing.js");
    ...
    const { collectDoctorPreviewNotes } = await import("./shared/preview-warnings.js");
