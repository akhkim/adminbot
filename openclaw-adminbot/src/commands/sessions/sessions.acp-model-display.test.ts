 ...
 *
 * `resolveSessionDisplayModelRef` (`src/commands/sessions/sessions-display-model.ts:123-148`)
  ...
  it("RED: ACP control-plane session must NOT report the agent-configured model", async () => {
    ...
    expect(
      ...
      `ACP session ${ACP_SESSION_KEY} reports model="${row?.model}" — that is the agent-configured ` +
        ...
        `resolveSessionDisplayModelRef (src/commands/sessions/sessions-display-model.ts:123) has zero ` +
        `ACP-awareness; the call site at src/commands/sessions/sessions.ts should consult ` +
