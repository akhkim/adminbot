 ...
 *
 * Bug summary: `classifySessionKey` (defined twice — `src/commands/sessions/sessions.ts:136-152`
 * and `src/commands/status/status.summary.runtime.ts:129-145`) classifies a session
 ...
 *   - `src/commands/sessions/sessions.ts:136-152`        (called by `sessionsCommand`,
 *     the path under test here)
 *   - `src/commands/status/status.summary.runtime.ts:129-145`
 ...
 * (mirroring `src/commands/sessions/sessions.test.ts` and `sessions.acp-runtime-metadata.test.ts`).
 */
...
describe("sessionsCommand kind classification (catalog #19)", () => {
  ...
  it("RED: ACP spawn-child session must NOT be classified as 'direct'", async () => {
    ...
    // plainly spawned from a group/topic. See `src/commands/sessions/sessions.ts:136-152`.
    ...
    expect(
      ...
      `ACP spawn-child session ${ACP_SPAWN_CHILD_KEY} is misclassified: kind="${row?.kind}". ` +
        ...
        `src/commands/sessions/sessions.ts:136-152 ignores these fields and returns "direct".`,
    ).not.toBe("direct");
  });
  ...
  it("RED (fix-shape): ACP spawn-child session should resolve to 'spawn-child'", async () => {
    ...
    expect(
      ...
      `ACP spawn-child session ${ACP_SPAWN_CHILD_KEY} should classify as "spawn-child" ` +
        ...
        `Fix locations: src/commands/sessions/sessions.ts:136-152 AND ` +
        `src/commands/status/status.summary.runtime.ts:129-145 (the same logic is duplicated; ` +
