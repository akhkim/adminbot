 ...
 * `src/commands/sessions/sessions.ts:294` and proves the bug in two parts:
 *
 ...
 * `src/gateway/sessions/session-utils.ts`) so it can pass session-key context to
...
/**
 * Mirror the per-row computation from `src/commands/sessions/sessions.ts:290-298`:
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    expect(
      ...
      `ACP session ${ACP_SESSION_KEY} should resolve to runtime id "acpx" (or the canonical ACP runtime label). ` +
        ...
        `(a) override at the call site in src/commands/sessions/sessions.ts:294 once isAcpSessionKey(row.key) is true, or ` +
