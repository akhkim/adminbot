...
describe("dashboardCommand", () => {
  ...
  it("opens configured hosted Control UI with gateway and token context", async () => {
    readConfigFileSnapshotMock.mockResolvedValue({
      path: "/tmp/openclaw.json",
      exists: true,
      raw: "{}",
      ...
      config: {
        gateway: {
          ...
          controlUi: {
            launchUrl: "https://dash.example.com/admin?theme=dark",
          },
        },
      },
      issues: [],
    ...
    resolveGatewayPortMock.mockReturnValue(18789);
    resolveControlUiLinksMock.mockReturnValue({
      httpUrl: "http://10.0.0.5:18789/openclaw/",
      wsUrl: "ws://10.0.0.5:18789/openclaw",
    });
    copyToClipboardMock.mockResolvedValue(true);
    detectBrowserOpenSupportMock.mockResolvedValue({ ok: true });
    openUrlMock.mockResolvedValue(true);
    ...
    const displayUrl =
      "https://dash.example.com/admin?theme=dark&gatewayUrl=ws%3A%2F%2F10.0.0.5%3A18789%2Fopenclaw";
    const authedUrl = `${displayUrl}#token=abc123`;
    expect(ensureGatewayReadyForOperationMock).toHaveBeenCalledWith({
      runtime,
      operation: "open the dashboard",
      yes: undefined,
      probeUrl: "ws://10.0.0.5:18789/openclaw",
      readyWhenReachable: true,
    });
