...
import type { EmbeddedAgentRunResult } from "../embedded/embedded-agent-runner.js";
...
import { buildAssistantMessage, buildUsageWithNoCost } from "../transport/stream-message-shared.js";
...
async function finalizeCliContextEngineTurn(params: {
  ...
  output: Awaited<ReturnType<typeof import("./execute.runtime.js").executePreparedCliRun>>;
}): Promise<void> {
...
async function runCliAgentInternal(params: RunCliAgentParams): Promise<EmbeddedAgentRunResult> {
  ...
  if (params.cleanupCliLiveSessionOnRunEnd === true) {
    try {
      const { closeClaudeLiveSessionForContext } = await import("./claude-live-session.js");
  ...
  if (params.cleanupBundleMcpOnRunEnd === true) {
    try {
      const { closeMcpLoopbackServer } = await import("../../gateway/protocol/mcp-http.js");
