...
} from "../../infra/exec/exec-control-command-guard.js";
...
import {
  ...
  requestGogKeyringPassword,
} from "../auth/gog-keyring-password.js";
...
import { type AgentToolWithMeta, failedTextResult, textResult } from "./common.js";
...
async function resolveGogKeyringPasswordEnv(params: {
  command: string;
  config?: ExecToolDefaults["config"];
  agentId?: string;
  sessionKey?: string;
  existingEnv?: Record<string, string>;
}): Promise<Record<string, string> | undefined> {
  if (!commandUsesGog(params.command) || params.existingEnv?.[GOG_KEYRING_PASSWORD]) {
    return undefined;
  }
  const password = await requestGogKeyringPassword({
    command: params.command,
    config: params.config,
    agentId: params.agentId,
    sessionKey: params.sessionKey,
  });
  return password ? { [GOG_KEYRING_PASSWORD]: password } : undefined;
}
...
export function createExecTool(
  ...
  const prepareParamsWithResolvedExecEnv = async (
    ...
    const rawPluginEnv =
      hookRunner?.hasHooks("resolve_exec_env") && typeof hookRunner.runResolveExecEnv === "function"
        ? await hookRunner.runResolveExecEnv(
            {
    ...
    const gogEnv = await resolveGogKeyringPasswordEnv({
      command: params.command,
    ...
    return markResolveExecEnvPrepared(params, {
      host,
      ...(resolvedEnv ? { pluginEnv: resolvedEnv } : {}),
    });
  };
  const autoReviewer =
