...
async function showControlUiDashboardNote(params: {
  ...
  customLaunchUrlConfigured?: boolean;
  ...
  if (browserSupport.ok) {
    ...
    if (!opened) {
      openHint = params.customLaunchUrlConfigured
        ? t("wizard.finalize.customDashboardOpenHint")
        : formatControlUiSshHint({
...
export async function finalizeSetupWizard(
  ...
  const configuredLaunchUrl = resolveControlUiLaunchUrl(
    nextConfig.gateway?.controlUi?.launchUrl ?? baseConfig.gateway?.controlUi?.launchUrl,
  );
  ...
  await prompter.note(
    [
      t("wizard.finalize.webUiUrl", { url: webUiUrl }),
      settings.authMode === "token" && settings.gatewayToken && !suppressGatewayTokenOutput
  ...
  if (!opts.skipUi) {
    ...
    if (hatchChoice === "tui") {
      ...
      const dashboard = await showControlUiDashboardNote({
        ...
        authedUrl,
        controlUiBasePath,
        customLaunchUrlConfigured: Boolean(configuredLaunchUrl),
        hintToken:
