...
import { OperatorSecretManager } from "./operator-secret-manager.js";
...
export function createGatewayAuxHandlers(params: {
  ...
  const operatorSecretManager = new OperatorSecretManager();
  let operatorSecretHandlersPromise: Promise<GatewayRequestHandlers> | null = null;
  const loadOperatorSecretHandlers = () =>
    (operatorSecretHandlersPromise ??= import("./server-methods/operator-secret.js").then(
      ({ createOperatorSecretHandlers }) => createOperatorSecretHandlers(operatorSecretManager),
    ));
  ...
  return {
    ...
    extraHandlers: {
      ...
      "operator.secret.list": createLazyHandler("operator.secret.list", loadOperatorSecretHandlers),
      "operator.secret.request": createLazyHandler(
        "operator.secret.request",
        loadOperatorSecretHandlers,
      ),
      "operator.secret.resolve": createLazyHandler(
        "operator.secret.resolve",
        loadOperatorSecretHandlers,
      ),
