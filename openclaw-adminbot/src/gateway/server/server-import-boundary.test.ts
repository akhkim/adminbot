...
describe("gateway startup import boundaries", () => {
  it("keeps heavy cron and doctor legacy paths out of the server.impl import graph", () => {
    const serverImpl = readSource("src/gateway/server/server.impl.ts");
    ...
    expect(readSource("src/gateway/server/server-runtime-subscriptions.ts")).toContain(
      'import("./server-session-key.js")',
    );
    expect(readSource("src/gateway/server/server-shared-auth-generation.ts")).not.toContain(
      'from "../config-reload.js"',
    );
    expect(readSource("src/gateway/server/server-aux-handlers.ts")).not.toContain(
      'from "../config-reload.js"',
    );
    expect(readSource("src/gateway/server/server-runtime-state.ts")).not.toContain(
    ...
    expect(readSource("src/gateway/server/server-reload-handlers.ts")).not.toContain(
    ...
    expect(readSource("src/gateway/server/server-aux-handlers.ts")).not.toMatch(
