...
import { isNixMode, normalizeStateDirEnv } from "../../config/paths/paths.js";
import { applyConfigOverrides } from "../../config/runtime/runtime-overrides.js";
...
import { createNodeReapprovalCoordinator } from "../node/node-reapproval-coordinator.js";
import {
...
import { resolveHookClientIpConfig } from "./hook-client-ip-config.js";
...
async function closeMcpLoopbackServerOnDemand(): Promise<void> {
  const { closeMcpLoopbackServer } = await import("../protocol/mcp-http.js");
    ...
    const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] =
      await startupTrace.measure("gateway.ws-imports", () =>
        Promise.all([
          ...
          import("./plugins-http/route-capability.js"),
        ]),
      );
  ...
  return {
    close: async (optsLocal) => {
      try {
        ...
        const { runGlobalGatewayStopSafely } =
          await import("../../plugins/hooks/hook-runner-global.js");
