...
describe("control UI routing", () => {
  it("renders responsive navigation shell, drawer, and collapsed states", async () => {
    ...
    // Any sidebar entry proves the shell drew its nav. This was /dreaming, which is no longer in
    // the sidebar -- see the note in navigation.ts: Workboard, Instances and Dreams are deliberately
    // absent because this deployment has no second instance and no use for idle-time dreaming.
  ...
  // out of the navigation in the dead-surface cleanup -- this deployment runs one gateway for one
  // lab and has no use for an idle-time dreaming loop -- so /dreaming no longer resolves to a tab
  // and both tests were asserting against a route that now falls through to the dashboard.
