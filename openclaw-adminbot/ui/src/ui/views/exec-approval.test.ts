...
describe("approval and confirmation modals", () => {
  ...
  it("renders exec approval as a labelled modal", async () => {
    ...
    expect(container.querySelector(".exec-approval-list")?.textContent).toContain("echo hello");
  });
  ...
  it("renders exec approval chrome from the active locale", async () => {
    ...
    expect(
      Array.from(container.querySelectorAll(".exec-approval-list__item strong")).map((entry) =>
        entry.textContent?.trim(),
      ),
    ).toEqual(["pnpm check:changed", "pnpm check:changed"]);
