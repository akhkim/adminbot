...
// conference/workshop deadlines with AoE-correct dates and days-left. The full
// live per-second countdown board is served by the AdminBot service at
// GET /deadlines (see docs/tools/adminbot-deadlines.md).
...
type Row = { venue: DeadlineVenue; instant: number; days: number };
...
export function renderDeadlines() {
  ...
  const rows: Row[] = DEADLINE_VENUES.map((venue) => ({
    ...
    .map((r) => ({ ...r, days: Math.floor((r.instant - now) / MS_DAY) }))
  ...
  return html`
    <style>
      ...
      .deadline-row {
        display: grid; grid-template-columns: 132px 84px 1fr auto; align-items: center; gap: 12px;
      ...
      .deadline-row .dl-days { font-variant-numeric: tabular-nums; font-weight: 600; color: var(--u); font-size: 13px; }
    ...
    <section class="deadlines-view">
      <p class="intro">
        Upcoming conference &amp; workshop deadlines (times AoE, UTC&#8209;12). For the full live
        per&#8209;second countdown board, open the AdminBot service at <code>/deadlines</code>.
      </p>
          ...
          : rows.map(
              ({ venue, days }) => html`
                <div class="deadline-row" style=${`--u:${urgencyColor(days)}`}>
                  ...
                  <span class="dl-days">${days}d left</span>
                  <span class="dl-name">
                    ${venue.name}
                    ${venue.venue_type === "workshop"
                      ? html`<span class="dl-note"> · NeurIPS 2026 workshop</span>`
                      : nothing}
                  </span>
                  ${venue.link
