...
const adminBotStepLabels: Record<string, string> = {
...
function formatAdminBotTime(value: string | number | null | undefined): string {
  if (typeof value === "number") {
    return Number.isFinite(value) ? formatRelativeTimestamp(value) : "n/a";
  }
  if (!value) {
    return "n/a";
  }
...
function renderAdminBotRail(
  adminBot: NonNullable<ChatProps["adminBot"]> | undefined,
): TemplateResult | typeof nothing {
  if (!adminBot) {
    return nothing;
  }
  ...
  return html`
    <aside class="chat-adminbot-rail" aria-label="AdminBot operations">
      <div class="chat-adminbot-rail__header">
        <div class="chat-adminbot-rail__title">
          <span class="chat-workspace-rail__eyebrow">AdminBot</span>
          <strong>Operations</strong>
        </div>
        <button
          class="btn btn--ghost btn--sm chat-workspace-rail__refresh"
          type="button"
          title="Refresh AdminBot"
          aria-label="Refresh AdminBot"
          ?disabled=${adminBot.loading}
          @click=${adminBot.onRefresh}
        >
          ${icons.refresh}
        </button>
      </div>
      ...
      ${adminBot.notice
        ? html`
            <div
              class="chat-adminbot-rail__notice chat-adminbot-rail__notice--${adminBot.notice.kind}"
            >
              ${adminBot.notice.text}
            </div>
          `
        : nothing}
      ${adminBot.error
        ? html`
            <div class="chat-adminbot-rail__notice chat-adminbot-rail__notice--error">
              ${adminBot.error}
            </div>
          `
        : nothing}
      ...
      <section class="chat-adminbot-rail__section">
        <div class="chat-adminbot-rail__section-title">
          <span>${icons.check}</span>
          <strong>Approvals</strong>
        </div>
        ${proposals.length === 0
          ? html`
              <div class="chat-adminbot-rail__empty">
                ${adminBot.loading ? "Loading approvals..." : "No pending actions"}
              </div>
            `
              ...
              return html`
                <article class="chat-adminbot-action">
                  <div class="chat-adminbot-action__top">
                    <span class="chat-adminbot-risk chat-adminbot-risk--${proposal.risk_tier}">
                      ${proposal.risk_tier}
                    </span>
                    <strong>${proposal.summary}</strong>
                  </div>
                  <div class="chat-adminbot-action__meta">
                    ${approvals}/${required} approvals - ${formatAdminBotTime(proposal.updated_at)}
                  </div>
      ...
      <section class="chat-adminbot-rail__section">
        <div class="chat-adminbot-rail__section-title">
          ...
          <strong>Paper pipeline</strong>
        </div>
        ${nudges.length > 0
          ? nudges.map(
              (nudge) => html`
                <article class="chat-adminbot-nudge">
                  <strong>${nudge.title}</strong>
                  <span>
                    ${nudge.type === "head_professor_escalation" ? "Escalate" : "Nudge"} -
                    ${adminBotStepLabels[nudge.step] ?? nudge.step}
                  </span>
                </article>
              `,
            )
            ...
            ? papers.map(
                (paper) => html`
                  <article class="chat-adminbot-nudge">
                    ...
                    <span>${adminBotStepLabels[paper.current_step] ?? paper.current_step}</span>
                  </article>
                `,
              )
            : html`
                <div class="chat-adminbot-rail__empty">
                  ${adminBot.loading ? "Loading papers..." : "No active papers"}
                </div>
              `}
      </section>
    </aside>
      ...
      ${hiddenCount > 0
        ? html`<button
            ...
            @click=${(e: Event) => {
              ...
              updateSlashMenu(draft, props, requestUpdate);
            }}
          >
    ...
    >
      ...
      <div
        class="chat-workbench ${
          props.sessionWorkspace?.collapsed && !props.adminBot
            ? "chat-workbench--workspace-collapsed"
            : ""
        }"
      >
        <div class="chat-side-rails">
          ${renderAdminBotRail(props.adminBot)} ${renderSessionWorkspaceRail(props.sessionWorkspace)}
        </div>
