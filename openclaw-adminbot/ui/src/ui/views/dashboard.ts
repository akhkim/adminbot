...
// One thing waiting on the viewer. `detail` is optional supporting text -- the onboarding item
// uses it for the step list; the queue items say everything in their summary.
type AttentionItem = {
  id: string;
  title: string;
  summary: string;
...
// How many remaining onboarding steps to name before the checklist speaks for itself.
const ONBOARDING_PREVIEW_LIMIT = 4;
...
function onboardingItem(state: AppViewState): AttentionItem | null {
  const onboarding = state.adminBotOnboarding;
  if (!onboarding) {
    return null;
  }
  const remaining = onboarding.remaining ?? [];
  const total = onboarding.steps?.length ?? 0;
  if (!remaining.length || !total) {
    return null;
  }
  const done = total - remaining.length;
  const preview = remaining.slice(0, ONBOARDING_PREVIEW_LIMIT);
  return {
    id: "onboarding",
    title: t("dashboard.onboarding.title"),
    summary: t("dashboard.onboarding.progress", { done: String(done), total: String(total) }),
    actionLabel: t("dashboard.onboarding.open"),
    // The full checklist is already a screen of its own; the dashboard says it is waiting and
    // hands off rather than rebuilding it here.
    onAction: () => {
      state.adminBotWelcomeVisible = true;
    },
    detail: html`
      <ul class="dashboard-card__steps">
        ${preview.map((step) => html`<li class="dashboard-card__step">${step.label}</li>`)}
...
function attentionItems(state: AppViewState, role: AccessRole): AttentionItem[] {
  // Own-account work first: a person can always act on their own onboarding, whereas a queue may
  // be someone else's to clear.
  return [onboardingItem(state), proposalsItem(state, role), registrationsItem(state, role)].filter(
