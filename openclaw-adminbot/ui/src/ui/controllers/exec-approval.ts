...
export function parseOperatorSecretRequested(payload: unknown): ExecApprovalRequest | null {
  if (!isRecord(payload)) {
    return null;
  }
  const id = normalizeOptionalString(payload.id) ?? "";
  ...
  const title = normalizeOptionalString(request.title) ?? "";
  const description = typeof request.description === "string" ? request.description : null;
  const variableName = typeof request.variableName === "string" ? request.variableName : null;
  if (!title || !variableName) {
    return null;
  }
  return {
    id,
    ...
    secretVariableName: variableName,
    createdAtMs,
    expiresAtMs,
  };
}
...
export async function refreshPendingApprovalQueue(state: ExecApprovalPromptState): Promise<void> {
  ...
  try {
    const [execResult, pluginResult, secretResult] = await Promise.allSettled([
      client.request("exec.approval.list", {}),
      client.request("plugin.approval.list", {}),
      client.request("operator.secret.list", {}),
    ]);
    const execApprovals =
    ...
    const secretRequests =
      secretResult.status === "fulfilled"
        ? (parseApprovalList(secretResult.value, parseOperatorSecretRequested) ?? [])
        : currentApprovalsForKind(state.execApprovalQueue, "secret");
    const refreshed = mergeRefreshedApprovalQueue(
      sortApprovalsNewestFirst([...execApprovals, ...pluginApprovals, ...secretRequests]),
      refreshStartedWith,
