...
describe("parseOperatorSecretRequested", () => {
  it("parses a pending secret input request", () => {
    const result = parseOperatorSecretRequested({
      id: "secret-1",
      createdAtMs: 1000,
      expiresAtMs: 120_000,
      request: {
        title: "GOG keyring password required",
        description: "Enter GOG_KEYRING_PASSWORD.",
    ...
    expect(result).toMatchObject({
      id: "secret-1",
      kind: "secret",
      secretTitle: "GOG keyring password required",
      secretVariableName: "GOG_KEYRING_PASSWORD",
      request: {
        command: "GOG keyring password required",
        agentId: "agent-1",
        sessionKey: "sess-1",
      },
    });
...
describe("refreshPendingApprovalQueue", () => {
  ...
  it("keeps refreshed approvals pending after their former expiry", async () => {
    vi.useFakeTimers();
  ...
  it("keeps active prompt errors when the former expiry passes", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-05-25T00:00:00.000Z"));
    try {
  ...
  it("requeues approvals returned by refresh lists even after their former expiry", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-05-25T00:00:00.000Z"));
    try {
      const request = vi.fn<RequestFn>(async (method) => {
      ...
      expect(state.execApprovalQueue.map((entry) => entry.id)).toEqual(["approval-expired-1"]);
    } finally {
      vi.useRealTimers();
    }
  });
