...
export type MemberAuthHost = {
  ...
  // whenever there is an onboarding checklist at all -- it is not tied to step completion.
...
async function applyMemberSession(host: MemberAuthHost, session: MemberSession) {
  ...
  // The dashboard warning card is driven straight off this on every login -- not a one-time
  // "auto-show" like the old popup, since the whole point is that it keeps warning until
  // explicitly acknowledged, reload after reload.
...
// Called from the dashboard warning card's "I have read this" button. Unlike the old popup's
// dismiss, this is the only way the card goes away — closing/navigating off the dashboard does
// not count, and neither does finishing every step (see MANDATORY_PROFILE_FIELDS-style framing
// in dashboard.ts: acknowledgement and completion are tracked separately on purpose).
export function acknowledgeOnboardingChecklist(host: MemberAuthHost): void {
  if (host.memberId) {
    markOnboardingChecklistAcknowledged(host.memberId);
  }
  host.adminBotOnboardingAcknowledged = true;
}
