...
const mine = {
  projects: [{ id: "p1", title: "Project p1" }],
  requests: [project()],
  interests: [],
};
...
async function mount(
  fetcher = vi.fn(async (url: string) =>
    response(url.includes("/discover?") ? { requests: [project()], next_cursor: null } : mine),
  ),
) {
  vi.useFakeTimers();
  ...
  const fetcher = vi.fn(async (url: string) =>
    response(
      url.includes("/discover?")
        ? {
            requests: [project(url.includes("cursor=") ? "p2" : "p1")],
...
it("keeps a failed save draft and refreshes managed/discovery state after retry", async () => {
  ...
  const fetcher = vi.fn(async (url: string, init?: RequestInit) => {
    ...
    return response(
      url.includes("/discover?") ? { requests: [project()], next_cursor: null } : mine,
    );
  });
  const { el } = await mount(fetcher);
  const select = el.querySelector<HTMLSelectElement>("form select")!;
  select.value = "p1";
  select.dispatchEvent(new Event("change"));
  const note = el.querySelector<HTMLTextAreaElement>("form textarea")!;
  note.value = "Retained draft";
  note.dispatchEvent(new Event("input"));
  el.querySelector("form")!.dispatchEvent(new Event("submit", { cancelable: true }));
  await vi.advanceTimersByTimeAsync(0);
  await el.updateComplete;
  expect(note.value).toBe("Retained draft");
  ...
  expect(
    fetcher.mock.calls.some(
      ([url, init]) => url.endsWith("/requests/p1") && init?.method === "PUT",
    ),
  ).toBe(true);
});
it("retrieves an off-page project directly and focuses its card", async () => {
  const { el, fetcher } = await mount(
    vi.fn(async (url: string) =>
      response(
        url.includes("/projects/")
          ? { request: project("p99") }
          : url.includes("/discover?")
            ? { requests: [project()], next_cursor: null }
            : mine,
      ),
    ),
  );
  ...
  try {
    ...
    expect(fetcher.mock.calls.some(([url]) => url.endsWith("/projects/p99"))).toBe(true);
  } finally {
  ...
  const { el } = await mount(
    vi.fn(async (url: string) => {
      if (url.includes("cursor=")) {
        ...
        return response({ requests: [project("p2")], next_cursor: null });
      }
      return response(
        url.includes("/discover?") ? { requests: [project()], next_cursor: "next" } : mine,
      );
    }),
  );
  const more = () =>
    [...el.querySelectorAll("button")].find((b) => b.textContent?.includes("Show more"))!;
  more().click();
  await vi.advanceTimersByTimeAsync(0);
  await el.updateComplete;
  expect(el.querySelectorAll("[data-project]")).toHaveLength(1);
...
it("keeps an offer draft after failure and withdraws a private saved offer", async () => {
  ...
  const fetcher = vi.fn(async (url: string, init?: RequestInit) => {
    ...
    return response(
      url.includes("/discover?")
        ? { requests: [{ ...project(), can_manage: false }], next_cursor: null }
        : { ...mine, projects: [], requests: [], interests: [own] },
    );
  });
  const { el } = await mount(fetcher);
  const form = el.querySelector<HTMLFormElement>('form[aria-label="Offer for Project p1"]')!;
  expect(form.closest("[data-project]")?.getAttribute("data-project")).toBe("p1");
  const note = form.querySelector("textarea")!;
  note.value = "Keep my offer";
  note.dispatchEvent(new Event("input"));
  form.dispatchEvent(new Event("submit", { cancelable: true }));
  await vi.advanceTimersByTimeAsync(0);
  await el.updateComplete;
  expect(note.value).toBe("Keep my offer");
  expect(el.textContent).toContain("Offline offer");
  const withdraw = [...el.querySelectorAll("button")].find((b) =>
  ...
  expect(
    fetcher.mock.calls.some(
      ([url, init]) => url.endsWith("/interest/withdraw") && init?.method === "POST",
    ),
  ).toBe(true);
});
...
it("focuses the request editor instead of sorting when editing a card", async () => {
  const { el } = await mount();
  [...el.querySelectorAll("button")].find((b) => b.textContent?.trim() === "Edit request")!.click();
  await el.updateComplete;
  await Promise.resolve();
  expect(document.activeElement).toBe(el.querySelector("form[data-help-editor] select"));
});
