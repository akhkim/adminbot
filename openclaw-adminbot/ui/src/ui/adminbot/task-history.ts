...
export async function listRecentTasks(
  ...
  if (token && token !== "visitor") headers.set("Authorization", `Bearer ${token}`);
  else {
    try {
      const visitor = sessionStorage.getItem(`adminbot-visitor:${baseUrl}`);
      if (visitor) headers.set("X-AdminBot-Visitor", visitor);
    } catch {
  ...
  if (!response.ok) throw new Error("Recent tasks could not be loaded.");
