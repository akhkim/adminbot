...
- If `gateway.controlUi.launchUrl` is set, `dashboard` opens that hosted
  Control UI URL instead of the bundled Gateway-served page and appends the
  current Gateway WebSocket URL as `gatewayUrl`.
- For Vercel-hosted Control UI deployments, use the repo root `vercel.json`;
  it builds `pnpm ui:build` and publishes `dist/control-ui`.
