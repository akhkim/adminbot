...
| Script tests    | `node scripts/test-projects.mjs test/scripts`       | 20 files / 113 tests; 5 failures: adminbot-reimbursement-from-email ×1, aurora-qwen35-setup ×2, aurora-runtime-bootstrap ×3. (adminbot-email-automation used to die at import, then failed one assertion; it is green now.) (Was 18/89 before `check-layering.test.ts` and `check-dir-size.test.ts` added 24 passing tests; the failure set is unchanged.) |
...
- ~~The `openclaw/plugin-sdk/media-runtime` export subpath resolves to nothing~~ —
  ...
  `test/scripts/adminbot-email-automation.test.ts` now collects and runs, and is
  green as of the trusted-sender calendar change: its one surviving assertion
  failure was the file's own `beforeAll` env stub, which the config's env
  restoration cleared after the first test, so every later test ran against an
  unconfigured deployment where no sender is privileged. Stubbing per test fixed
  it. Script tests are now 5 failures, not 6.
