...
You can host the same Control UI build outside the Gateway, for example on Vercel, and tell OpenClaw to open that URL by default:

```json
{
  "gateway": {
    "controlUi": {
      "launchUrl": "https://your-control-ui.vercel.app",
      "allowedOrigins": ["https://your-control-ui.vercel.app"]
    }
  }
}
```

`gateway.controlUi.launchUrl` only changes what setup and `openclaw dashboard` open. The Gateway still serves the bundled UI unless `gateway.controlUi.enabled` is false, and the hosted origin still needs to be allowed with `gateway.controlUi.allowedOrigins`.

When OpenClaw launches the hosted UI, it appends `gatewayUrl=<ws-or-wss-url>` so the browser knows which Gateway WebSocket to connect to, and it adds token auth in the URL fragment when the token can be safely resolved. For HTTPS-hosted apps, expose the Gateway over `wss://` or through an HTTPS/TLS proxy; browsers may block an HTTPS page from connecting to a plaintext `ws://` Gateway.

### Vercel deployment

This repo includes a root `vercel.json` for hosting the Control UI as a static app:

- install command: `corepack pnpm install --frozen-lockfile`
- build command: `corepack pnpm ui:build`
- output directory: `dist/control-ui`
- SPA rewrite: all routes fall back to `/index.html`

After Vercel creates the deployment URL, add that origin to your OpenClaw config:
...
Then run `openclaw dashboard`. The command opens the Vercel URL and supplies the active Gateway WebSocket URL as `gatewayUrl`.
