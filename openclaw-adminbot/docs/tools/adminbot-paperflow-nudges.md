...
2. The email says what it wants and names the bot mailbox: *bcc us on the notification the venue
   sent*.
3. The bcc lands in that mailbox. The hourly email pass classifies it, matches it to the paper, and
...
mail already exists, the author is already reading it, and adding a bcc is the smallest thing we
can ask.
...
rows disagree about middle names as a rule — the roster says *Rahul Shrestha*, the papers say
*Rahul Babu Shrestha* — and exact matching would route every one of those papers to nobody while
...
| --- | --- | --- | --- |
| `reviews_out` | `RV` | The submission page is on file | A bcc'd review notification |
| `rebuttal` | `RB` | Reviews evidenced, no rebuttal doc on file | A bcc'd rebuttal, or the doc slot |
| `decision` | `DC` | Rebuttal accounted for | A bcc'd decision mail |
| `camera_ready` | `CM` | `venue_decision` is `accept` | A bcc'd camera-ready confirmation |
| `conference` | `CA` | `venue_decision` is `accept` | A bcc'd registration or booking |
...
| --- | --- | --- |
| `ADMINBOT_BOT_EMAIL` | yes | The mailbox the emails tell authors to bcc. Without it the sweep returns 503 and sends nothing — a nudge whose whole payload is "bcc us at" is worse than silence without an address to name. |
...
- **The venue ladder, which fills itself in.** Reviews → Rebuttal → Decision → Camera ready →
  ...
  there is nothing a person can do to make reviews arrive; the one action available is the bcc,
...
Above the checklist sits what the paper *is*, rather than what has been done to it, all three
...
## Why a bcc can be trusted, and where it is not
