...
| Add a caller of the local model | route it through `runGated` in `extensions/adminbot/src/inference/gate.ts` — never a bare `fetch` to the GPU                                                               | [ADR-0008](adr/0008-the-task-runner-owns-the-request.md)                 |
| Make a workflow resumable       | register it on the runner in `extensions/adminbot/src/tasks/http-handlers.ts` and wrap its steps in `ctx.step` / `ctx.commit`                                              | [ADR-0008](adr/0008-the-task-runner-owns-the-request.md)                 |
