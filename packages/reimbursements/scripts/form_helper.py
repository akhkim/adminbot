...
BUNDLED_FORMS = Path(__file__).resolve().parent.parent / "assets" / "waterloo_travel_v1"


def resolve_forms_dir() -> Path:
    """Use an explicit operator pack override or the versioned bundled pack."""
...
def extract_receipt(path: Path) -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            "PDF extraction requires the packages/reimbursements Python dependencies"
        ) from error
    text_parts: list[str] = []
    images: list[dict] = []
    try:
...
def require_template(name: str) -> Path:
    ...
    if not template.is_file():
        ...
        raise FileNotFoundError(
            ...
            f"(looked in {FORMS}; set ADMINBOT_REIMBURSEMENT_FORMS_DIR or ship it at "
            f"{BUNDLED_FORMS})"
        )
    return template
...
def fill_workbook(data: dict, output: Path) -> None:
    try:
        ...
        raise RuntimeError(
            "form generation requires the packages/reimbursements Python dependencies"
        ) from error
